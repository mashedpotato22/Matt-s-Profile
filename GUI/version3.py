...
def board(player_name, score):
    ...
    player2=player(random.choice(('Joe','John','Jack')), random.randint(0, 10000)) # dummy variable
    player3=player(random.choice(('Joe','John','Jack')), random.randint(0, 10000)) # dummy variable
    player4=player(random.choice(('Joe','John','Jack')), random.randint(0, 10000)) # dummy variable
...
def check_guess(choice):
    ...
    if choice == "easy":
        if score>0:
            if guess == result:
                msg=f'Your guess is {guess}, the answer is {result} You won the Game \U0001f606!'
                nameinput('win') # win! go to leaderborad    
            elif guess > result:
                score -= 200
                msg=f'Your guess is {guess}, the answer is {result} Your answer is too big {int(score)} left'    
            else:
                score -= 200
                msg=f'Your guess is {guess}, the answer is {result} Your answer is too small {int(score)} left' 
        else:
            msg='You Lost!'
        ...
        if score>0:
            if guess == result:
                msg=f'Your guess is {guess}, the answer is {result}   You Won \U0001f606'
                nameinput('win') # win! go to leaderborad
            else:
                score -= 200
                msg=f'Your guess is {guess}, the answer is {result}  Nice Try \U0001f643 {int(score)} left'
        else:
            msg='You Lost!'
            nameinput('lost') # lost! go to leaderborad
